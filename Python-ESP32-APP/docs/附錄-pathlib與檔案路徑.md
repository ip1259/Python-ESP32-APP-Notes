...
### `Path` 與 `glob()`：表示與搜尋受控路徑

`Path("outputs")` 傳入路徑文字、回傳 Path 物件；`glob("*.csv")` 傳入搜尋模式、回傳可逐一走訪的路徑結果。搜尋前要限定在練習資料夾，不能對未知範圍遞迴掃描。

### `rename()`、`unlink()` 與 `rmdir()`：更名或移除路徑

`rename(target)` 會更名或移動路徑；`unlink()` 刪除檔案；`rmdir()` 只移除空資料夾。它們的效果比回傳值重要，可能使資料難以復原，務必先確認目標僅是剛建立的練習檔。
...
!!! warning "先確認目標；不符合就停止"
    只執行範例剛建立的 `practice_cleanup/delete_me.txt`。如果檔案不是練習檔、資料夾位置被你改過、目標不是檔案，或資料夾內還有其他檔案，請停止，不要刪除。不要把 `unlink()` 或 `rmdir()` 套用到重要專案檔、感測資料、專案根目錄或不確定的路徑。
...
預期結果：區塊內顯示 `暫存檔存在：True` 和 CSV 文字；離開 `with` 區塊後顯示 `暫存資料夾仍存在：False`。`TemporaryDirectory` 只適合可重新產生的暫存資料，不能用來保存重要專案檔、感測紀錄或需要長期保留的成果。
...
`write_text()` 的用途是寫入完整的新文字，會覆寫同名檔。請不要用它直接修改唯一的重要專案檔或感測資料；先複製一份，或改用新的練習檔名。
