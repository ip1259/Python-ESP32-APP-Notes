...
!!! warning "不要直接把 I2C 訊號拉到 5V"
    有些 LCD 背包在接 5V 時，會把 SDA、SCL 也拉到 5V。這可能損壞 ESP32 的 GPIO。若模組在 3.3V 下無法正常工作，先停止使用本頁接線，依模組板上標示與資料表確認工作電壓和是否需要電平轉換；不要自行把 VCC 改接 5V 後繼續使用原接線。
...
- 你能理解 SDA、SCL 是 I2C 訊號線，而位址不是 GPIO 編號。
...
雙向電平轉換器的用途，是讓 ESP32 保持在 `3.3V` 側，LCD 保持在 `5V` 側，兩邊只透過轉換器交換 I2C 訊號。不同模組的腳位標示與電源條件可能不同；本頁不提供 5V LCD 的接線表或操作步驟。

若你的 LCD 在 `3.3V` 無法正常使用，先停止接線，確認模組資料表、電平轉換器型號與電源條件；找到適合的選做教材前，不要自行依網路圖片改接。
...
想了解主線程式的初始化與兩行文字更新順序，可閱讀[延伸選讀：LCD 顯示程式導讀](附錄-LCD程式導讀.md)；這不影響本頁的完成條件。
