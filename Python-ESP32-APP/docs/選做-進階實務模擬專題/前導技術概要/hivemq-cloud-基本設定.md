...
不要使用 `#` 或 `+`。它們是 MQTT 的萬用字元，會把權限擴大到多個 topic，不適合這個練習。
...
若現在不接著閱讀 RFID 第 2 章的練習，請登出 Web Client，並在控制台刪除兩組測試帳密。只有在接續進行短時間測試時才保留帳密。
...
`#` 代表多層萬用字元，可能涵蓋 Broker 裡所有 topic。練習只需要一個完整 topic，範圍越小越不容易誤收或誤送資料。

## 重點整理

- HiveMQ Cloud Serverless 可作為 MQTT 的選做測試 Broker。
- 連線資料、帳密與 topic 都不應公開；只使用 TLS 與可刪除的測試資料。
- Serverless 的一組帳密對應一項權限，權限應限制在一個完整 topic。
- 若控制台或方案和本頁不同，停止操作並以官方文件為準。

## 下一步

- 回到[進階實務模擬專題](../index.md)了解 MQTT 在整體資料流中的位置。
- 要使用測試帳密收發練習訊息時，閱讀[第 2 章：ESP32 MQTT 假信封事件](../../選做-RFID專題/第2章/index.md)的固定假資料規則；不進行該練習時，刪除測試帳密。
