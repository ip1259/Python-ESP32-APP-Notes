...
這一頁帶你認識 AIoT 專案常見的 GUI 與 App 解決方案。目的是知道不同工具各自適合什麼情境，以及未來想深入時可以從哪裡開始；本頁**不要求安裝或完成 App**。
...
!!! note "這些是探索入口，不是選課要求"
    表格中的連結供你未來依興趣自行閱讀。你不需要現在比較語言、安裝開發環境，或決定要使用哪一個框架；本課仍以既定 ESP32 與 Python 主線的完成成果為準。
...
完成主線的資料流練習後，再依興趣閱讀「手機 App 與無線控制」中的 BLE 與 Thunkable 選做內容。
