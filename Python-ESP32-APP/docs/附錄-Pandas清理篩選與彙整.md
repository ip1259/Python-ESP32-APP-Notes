...
條件門檻必須符合你的分析目的；這裡的 `27.0` 只用於範例，不能當成所有環境的警戒標準。
...
回到[將感測資料存成 CSV 並繪製趨勢圖](04-python-感測資料視覺化.md)連結本頁的資料處理位置。接著可閱讀[延伸選讀：Matplotlib 圖表基本元件](附錄-Matplotlib圖表基本元件.md)，認識折線圖的標題、座標軸、圖例與格線。
