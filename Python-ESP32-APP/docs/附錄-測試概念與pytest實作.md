...
- 以 Python 的 `pytest`、`assert`、`raises()` 與參數化測試純函式。
- 判讀通過與失敗結果，並知道單元測試無法取代 ESP32 實機驗收。
...
| 單元測試 | 一個小函式的輸入／輸出是否符合契約？ | 盡量沒有外部依賴 | JSON 字典能否轉成合法資料列 |
| 整合測試 | 多個元件接起來是否相容？ | 檔案、序列埠、服務等受控依賴 | Python 能否讀到 ESP32 JSON |
| 端對端／系統測試 | 使用者完整流程是否可完成？ | 接近真實環境 | 按 Gradio 按鈕後看到資料並控制 LED |
| 監控與事故回顧 | 上線後是否持續可靠？ | 真實使用環境 | 正式產品才需要的日誌、告警與追蹤 |

!!! warning "單元測試通過不等於系統已驗收"
...
| 冒煙測試（smoke testing） | 最核心路徑是否能快速啟動與運作 | 新版本剛建立、部署或上課前 | ESP32 可燒錄、Python 可啟動、Gradio 可在 localhost 開啟 | 所有細節、錯誤情境與長時間可靠性 |
| 探索式測試（exploratory testing） | 人在操作中能否發現規格未列出的問題 | 介面完成後、整合前後 | 故意連按按鈕、改變輸入順序、觀察錯誤提示是否清楚 | 每次都能完全重現的自動化證據 |
| 驗收測試（acceptance testing） | 成果是否符合已約定的可交付條件 | 交付或評量前 | 依主線驗收確認感測、CSV、圖表與儀表板成果 | 內部每個函式與錯誤分支都已測過 |
...
- 全部通過：代表目前 8 個已寫下的契約仍符合，不代表沒有其他未寫的問題。
...
| fake | 可運作但簡化的替代實作 | 記憶體中的假資料儲存區 | 行為可能和正式服務仍有差異 |
| stub | 提供預先設定的回覆 | 固定回傳一筆成功 JSON | 不應拿來驗證複雜互動 |
| mock | 檢查某個外部合作對象是否被預期方式呼叫 | 確認程式嘗試傳送命令 | 過度檢查實作細節會讓重構困難 |

本頁不用 fake、stub 或 mock 的 API，因為純函式不需外部依賴。當未來測試 UART、時間、檔案或網路時，先判斷要實機整合測試、使用受控替身，還是根本不該把外部依賴放進單元測試。

## 覆蓋率與持續整合（CI）的正確定位

覆蓋率（coverage）表示測試執行時走過多少程式碼、分支或條件；它能協助發現「完全沒有被案例碰到」的區域，但高覆蓋率不代表斷言正確、需求完整、介面好用或硬體可靠。不要為了數字硬寫沒有意義的測試。

持續整合（Continuous Integration，CI）是把測試、格式檢查或建置等既有指令，在每次合併前或提交後自動重跑的流程。CI 有助於及早看到回歸，但不會自動理解需求，也不會替你接線、量測感測器或保護公開服務。本課不要求帳號或 CI 設定；目前在電腦上手動執行 `pytest -q`，就是建立將來可交給 CI 重跑的基礎。

## AIoT 最小測試策略對照

| 區域 | 優先驗證方式 | 可保存的證據 | 停止或轉交時機 |
| --- | --- | --- | --- |
| 資料轉換、範圍判斷 | 單元／回歸測試 | 測試案例與通過輸出 | 外部依賴出現時改用整合策略 |
| JSON、CSV、UART 資料流 | 受控整合與錯誤訊息檢查 | 固定輸入與輸出紀錄 | COM 埠衝突或格式不明時先停止並分流 |
| ESP32、DHT11、LED | 實機冒煙與驗收觀察 | 燒錄結果、序列輸出、硬體行為 | 接線、電壓或輸出異常時斷電檢查 |
| Gradio 本機介面 | 探索操作與主線驗收 | localhost 畫面與回呼結果 | 不要改成公開網址或遠端控制 |
...
一組有用的案例至少包含：正常資料、剛好在上下限的邊界資料，以及應被拒絕的錯誤資料。外部服務難以控制時，其他語言與工具也常使用 stub、fake 或 mock 等測試替身；它們的目的都是讓測試可重複，不是偽造產品已經能在真實環境工作。本頁先不使用替身 API。
...
`normalize_sensor()` 傳入一筆字典，回傳欄名固定、數值已轉成 `float` 的字典。缺欄位、無法轉數字、空白時間或超出範圍時，函式會主動丟出 `ValueError`；這是可測試的資料契約，不是直接把錯誤資料寫進 CSV。
...
- 能說明單元測試無法取代 UART、Gradio 與 ESP32 的整合或實機驗收。
...
- 單元測試把純函式的輸入／輸出契約固定下來，正常、邊界與錯誤案例缺一不可。
- 通過代表目前案例符合預期，不代表整個產品、硬體或網路都已驗收。
- 失敗輸出是縮小問題範圍的證據，應先看案例名稱與預期／實際差異。

## 下一步

回到[整合專題：完成可展示的環境看板](08-整合專題.md)，將純函式測試、UART 實機檢查與介面操作分開驗收。
