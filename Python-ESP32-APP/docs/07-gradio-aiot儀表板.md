...
| 執行位置 | Python／你的電腦；網頁只在同一台電腦的本機瀏覽器使用 |
...
| 本頁做法 | 在你的電腦的 localhost 操作 USB UART | 需要規劃前端、後端 API、資料庫、部署與監控 |
| 不應省略的工作 | 本頁不處理公開服務 | 帳號與權限、輸入驗證、資安、錯誤追蹤、測試、備份、效能與擴充 |

因此，不要把本頁範例程式直接當作公開網站或最終產品。正式網站通常還需要處理帳號、安全、資料保存與長期維護；可依需求再選擇合適的網頁技術。本頁只確認本機資料能顯示，且命令能回到 ESP32。
...
!!! note "為什麼先清除輸入緩衝區？"
    緩衝區是暫時排放尚未讀取資料的位置。ESP32 每約 2 秒會主動傳來 `sensor` 資料；送 LED 命令前清除舊訊息，可降低把「之前的感測資料」誤認成「這次命令回覆」的機會。送出後仍會等待 `type` 為 `status` 的 JSON，才判定命令成功。
...
當手動更新已穩定後，為頁面加入一個「溫度提醒」文字框：自行選擇一個練習門檻，例如 `30.0`；當最新溫度高於門檻時顯示提醒，否則顯示正常。回呼是按下按鈕後執行並回傳畫面文字的函式。先只修改 Python 回呼的回傳文字，不要加入新的硬體或公開網頁設定。

## 延伸選讀

想了解按鈕事件、最近資料與 COM 埠責任如何配合，以及本機原型的使用界線，可閱讀[延伸選讀：Gradio 事件、狀態與原型邊界](附錄-Gradio事件狀態與原型邊界.md)。

想先用不連接 ESP32 的固定資料學習元件選擇與版面安排，可閱讀[延伸選讀：Gradio 介面、元件與版面](附錄-Gradio元件與事件.md)。

想了解按鈕、輸入變更與文字送出如何把資料交給 Python 並更新多個元件，可閱讀[延伸選讀：Gradio 事件、資料流與元件更新](附錄-Gradio事件資料流與元件更新.md)。

想練習短期介面狀態、門檻輸入檢查與清楚的錯誤提示，可閱讀[延伸選讀：Gradio 狀態、驗證與使用體驗](附錄-Gradio狀態驗證與使用體驗.md)。
