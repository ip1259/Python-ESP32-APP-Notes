...
| 板載 LED | GPIO 2 | 不需另外接線；若 LED 行為相反，先依下方常見問題確認板子型號與程式設定 |

!!! tip "DHT11 模組與裸 DHT11 不一樣"
    本教材使用的是 DHT11 模組，通常已包含需要的上拉電阻。若你拿到的是只有 4 根腳的裸感測器，先停止接線；它的腳位與是否需要額外電阻必須依該元件的資料表確認，不能直接照本頁接線。
...
部分相容開發板的板載 LED 可能是低電位亮起。先在 ESP32 程式中將 `HIGH`／`LOW` 對調後重新上傳，並用 LED 實際亮滅確認；不要只修改 Python 程式。若板子型號不明或結果不一致，停止修改並查閱板子標示或資料表。
