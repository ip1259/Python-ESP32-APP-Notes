...
### `list_ports.comports()`：列出候選埠

不需傳入值，回傳可逐一查看的埠資訊集合；每個項目有 `device`、`description` 等屬性。它只提供候選值，不保證哪一個是 ESP32，必須用拔插變化與主線固定 JSON 確認。

### `bytes.decode()`：把 bytes 顯示為文字

常見傳入編碼為 `"utf-8"`，回傳 `str`。`errors="replace"` 會將無法解碼的內容變為 `�`，只適合除錯顯示；含取代字元的資料不能當成感測值使用。
