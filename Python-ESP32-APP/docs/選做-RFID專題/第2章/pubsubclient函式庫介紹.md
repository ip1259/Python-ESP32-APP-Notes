...
PubSubClient 是 Arduino 常用的 MQTT Client（訊息用戶端）函式庫；第 2 章用它讓 ESP32 發布固定假信封，並訂閱相符回覆。

## 你會學到什麼

- 說明 PubSubClient 與 Wi-Fi、TLS Client（安全連線工具）、MQTT Broker（訊息中介服務）的分工。
- 看懂建立連線、訂閱、發布、接收與持續處理的順序。
- 認識第 2 章用到的主要函式與回傳結果。
- 知道本函式庫的訊息大小、QoS 與使用範圍限制。
...
可以記成六個動作：**準備網路工具 → 建立 PubSubClient → 指定訊息中介服務 → 指定收到訊息的處理函式 → 連線、訂閱與發布 → 持續呼叫 `loop()`**。
...
| `subscribe(topic, qos)` | 訂閱一個 topic | 完整 topic（訊息地址）與 QoS | 成功回傳 `true`。 |
| `publish(topic, payload, retained)` | 送出一筆訊息 | 完整 topic（訊息地址）、文字、是否保留 | 成功加入傳送流程時回傳 `true`。 |
...
第 2 章先略過帶有 `uid_envelope` 的自身假信封，再確認回覆帶有本次 `event_id` 和 `whitelisted`。只檢查狀態不夠，因為其他事件的回覆也可能在同一個 topic 出現。
...
PubSubClient 的官方 README 說明：它只能發布 QoS `0`，可訂閱 QoS `0` 或 `1`；預設最大封包大小是 `256` 位元組（bytes），也可用 `setBufferSize()` 調整。第 2 章傳送固定假信封，使用 QoS `0`、`retain: false` 與 `512` 位元組緩衝區。這能讓你觀察資料流，但不代表每筆訊息都保證送達或保存。 [官方 README](https://github.com/knolleary/pubsubclient)

!!! warning "函式庫的使用範圍"

    PubSubClient 的官方 README 說明此函式庫不再維護，並建議新專案選擇仍有維護的替代函式庫。本章固定使用 `2.8.0` 來理解短小的假信封流程；不要把它當成新產品的長期技術選擇。 [官方 README](https://github.com/knolleary/pubsubclient)

## 何時使用、何時不要使用

| 情境 | 建議 |
| --- | --- |
| 想理解小型 ESP32 如何發布與訂閱固定假信封 | 可依第 2 章使用固定版本。 |
...
- 第 2 章以固定假信封、單一專屬 topic、QoS `0` 與短期帳密縮小練習範圍。
- 函式庫不再維護；本頁介紹固定用法，不等於推薦新專案採用。
