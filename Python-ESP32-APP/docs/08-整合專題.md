...
## 選做實作：用手機瀏覽器短時間測試 Gradio Share

這是額外的選做實作，需要可上網的電腦與手機。先確認電腦 localhost 已成功運作，並只使用不含個人資料的練習資料。透過分享功能，手機可以開啟 Gradio 建立的暫時分享網址；手機仍然是先連線到電腦上的 Python Gradio 頁面，不是直接連到 ESP32。

```text
手機瀏覽器 → Gradio 分享網址 → Python → USB UART → ESP32
```

將 `integrated_dashboard.py` 結尾的 `share=False` 改為 `share=True`：
...
PowerShell 顯示 `https://...gradio.live` 類型的網址後，用手機瀏覽器開啟它。按「讀取並記錄資料」及 LED 按鈕，預期會看到與電腦 localhost 相同的最新讀值、資料表、圖表與板載 LED 回覆。

!!! warning "分享網址的使用範圍"
    分享網址可從網際網路開啟。請自行評估網址外流、頁面資料與按鈕可能被取得網址的人使用的風險；不要在頁面或程式放入密碼、token、個人資料或敏感環境資料。本補充只適合短時間測試感測資料與板載 LED，不適合作為長期服務或正式遠端控制。

測試完成後，在 PowerShell 按 `Ctrl+C` 停止程式，再將 `share=True` 改回 `share=False`。電腦上的整合儀表板仍必須是唯一開啟 ESP32 COM 埠的程式；測試前先關閉 Arduino 序列監控與其他 UART Python 程式。
...
| 錯誤處理 | 先拔除 USB，再暫時拔除 DHT11 DATA；接回後再供電 | LCD 與 UART 回報 DHT 讀取錯誤；恢復接線後重新有資料 | 看到錯誤後再確認資料恢復 |

最後一列是除錯練習；不確定接線時，先停止，依本頁接線表、元件標示與已確認資料表核對，不要反覆猜接腳位。
...
- 你能從自己的資料整理出一項圖表觀察、一項資料限制和一次除錯紀錄。
...
想依 LCD、UART、CSV、圖表與網頁的症狀判斷優先查看的位置，可閱讀[延伸選讀：整合專題故障分流](附錄-故障排除索引.md)。

## 下一步

下一步可以閱讀[LightBlue 與 ESP32 交換固定資料](選做-既有BLE-App測試.md)；這項 BLE 練習不影響主線成果。
