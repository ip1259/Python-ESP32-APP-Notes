...
## 延伸補充：用手機瀏覽器測試 Gradio Share

在電腦本機位址測試成功後可以開始測試Gradio APP的公開分享功能。透過分享功能可以使Gradio APP變成公開服務，手機便可以開啟 Gradio 建立的暫時分享網址。要注意，手機仍然是先連線到電腦的Python Gradio APP，不是直接連到 ESP32；資料與 LED 指令仍會經過電腦上的 `integrated_dashboard.py` 與 USB UART。
...
要改接線時，先拔除 USB。最後一列是除錯練習；不確定接線時，先停止，依本頁接線表、元件標示與已確認資料表核對，不要反覆猜接腳位。
